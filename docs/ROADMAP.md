...
| Reconciliation | Done locally | Deterministic matching, confidence, manual review and history | Invoice receipts, partial payments, tenant-safe payout allocation, and reviewed payout-to-bank confirmation pass PostgreSQL integration tests |
...
| Frontend | In progress | React/TypeScript authentication, organization setup, customers, and invoice creation/issuing are working; payments, banking, review, ledger and dashboards follow | Type/lint/build checks pass; live identity, organization, customer, and invoice workflow checked through the API proxy; responsive loading/empty/error states included; no frontend secrets |
