...
Owners and accountants can call `POST .../reconciliation/refresh` after a bank sync. Review cases are created only for posted USD inflows. Suggested amounts must use whole cents. Inside the date window from issuance through 30 days after the due date, an exact outstanding invoice balance scores `0.8500`. An exact balance whose description includes the invoice number scores `1.0000`. A smaller payment is suggested only when its description includes the invoice number, and scores `0.9000`. This keeps an unrelated small deposit from being suggested for every open invoice.

An imported Stripe payout is suggested when its exact net amount matches the deposit and the bank date falls from three days before through seven days after Stripe's arrival date. The amount-and-date rule scores `0.9500`; a bank description that also includes the Stripe payout ID scores `1.0000`. Same-amount payouts remain separate candidates for review.

Use `GET .../reconciliation/cases?status=OPEN` for the queue and `GET .../reconciliation/cases/{id}` for candidates and decision history. `reviewState` counts both invoice and payout candidates and says whether the open case has no candidate, one candidate, or multiple candidates. Invoice candidates include the original total and current outstanding balance; payout candidates include the provider ID, net amount, arrival date, score, and rule. All organization members can read the queue. An owner or accountant can send an invoice ID plus the current case version to `POST .../match`, send a payout ID plus the version to `POST .../match-payout`, or send the version to `POST .../ignore`.

Refreshing never posts money automatically. An accepted invoice match locks and rechecks the case, bank transaction, invoice, and live receivable balance, then records a balanced `BANK_PAYMENT` journal: debit CASH and credit RECEIVABLES. A partial payment leaves the invoice ISSUED with a smaller outstanding balance; the payment that clears the balance moves it to PAID. Multiple bank transactions can therefore settle one invoice. An accepted payout match rechecks the exact net amount, date window, and live transit balance, then records `PAYOUT_DEPOSIT`: debit CASH and credit PAYOUTS_IN_TRANSIT. A payout and a bank transaction can each be confirmed only once. An active Stripe attempt keeps the invoice out of bank matching, and a reviewed bank payment blocks a later full-amount Stripe intent; canceling an unpaid Stripe intent makes bank review available again. The matched target and amount are saved with the case and append-only decision. Imported bank history and existing journals remain unchanged.
