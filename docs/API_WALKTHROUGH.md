...
Owners and accountants can call `POST .../reconciliation/refresh` after a bank sync. Review cases are created only for posted USD inflows. Suggested amounts must use whole cents. Inside the date window from issuance through 30 days after the due date, an exact outstanding balance scores `0.8500`. An exact balance whose description includes the invoice number scores `1.0000`. A smaller payment is suggested only when its description includes the invoice number, and scores `0.9000`. This keeps an unrelated small deposit from being suggested for every open invoice.

Use `GET .../reconciliation/cases?status=OPEN` for the queue and `GET .../reconciliation/cases/{id}` for candidates and decision history. `reviewState` says whether the open case has no candidate, one candidate, or multiple candidates. Each candidate includes the original invoice total and its current outstanding balance. All organization members can read the queue. An owner or accountant can send an invoice ID plus the current case version to `POST .../match`, or send the version to `POST .../ignore`.

Refreshing never settles an invoice automatically. An accepted match locks and rechecks the case, bank transaction, invoice, and live receivable balance, then records a balanced `BANK_PAYMENT` journal: debit CASH and credit RECEIVABLES. A partial payment leaves the invoice ISSUED with a smaller outstanding balance; the payment that clears the balance moves it to PAID. Multiple bank transactions can therefore settle one invoice, while one bank transaction can be used only once. An active Stripe attempt keeps the invoice out of bank matching, and a reviewed bank payment blocks a later full-amount Stripe intent; canceling an unpaid Stripe intent makes bank review available again. The matched amount is saved with the case and append-only decision. Imported bank history and existing journals remain unchanged.
