...
A verified payout credits STRIPE_CLEARING for each gross charge, debits PROCESSING_FEES for Stripe's fee, and debits PAYOUTS_IN_TRANSIT for the net. It does not debit CASH because the imported Stripe report is separate from the bank statement. After Plaid imports the actual deposit, refresh the reconciliation queue and review its payout candidates. An accepted exact amount/date match creates a `PAYOUT_DEPOSIT` journal that debits CASH and credits PAYOUTS_IN_TRANSIT. Payout reversals, payout lines for refunds or adjustments, revenue credit notes, dispute fees, and dispute evidence submissions aren't implemented.
