...
The backend now supports users, organizations, customers, and invoices. I can log in, assign organization roles, create a draft invoice, edit it, issue it, and void it. Issuing and voiding now create balanced ledger postings and reversals. Stripe Sandbox payment code is implemented and tested locally; its account-backed sandbox check is pending. Bank connections and the frontend are still to build.
...
I'm finishing the account-backed Stripe Sandbox check next. Bank syncing, reconciliation, and a frontend dashboard follow.
...
- [Sandbox payments](docs/STRIPE_SANDBOX.md)
...
This checks Java formatting, compiles the app, runs 25 focused tests, packages an executable JAR, and runs 65 integration tests against isolated PostgreSQL through Testcontainers. Integration tests fail if Docker is unavailable rather than silently skipping.
